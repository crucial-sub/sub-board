...
- `/users/me/stats` API를 추가해 홈 화면 카드가 로그인 사용자의 실제 게시글/댓글/태그 수치를 기반으로 렌더링된다.
- SSE 기반 `notifications/stream`을 도입해 새 게시글·내 글에 달린 댓글을 실시간 토스트로 알려준다. 프론트는 `NotificationToaster`를 통해 이벤트를 받아 Zustand 스토어에 쌓고 자동으로 사라지는 알림을 렌더링한다.
